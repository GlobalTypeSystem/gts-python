...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--config", help="Path to optional GTS config JSON to override defaults"
    )
    p.add_argument(
    ...
    s.add_argument(
        "--from-id", required=True, help="GTS ID of instance or schema to be casted"
    )
    ...
    s.add_argument(
        "--gts-with-path",
        required=True,
        help="GTS ID with attribute path (e.g., gts.a.b.c.d.v1~@field.subfield)",
    )
    ...
    s.add_argument(
        "--out", required=True, help="Destination file path for OpenAPI spec JSON"
    )
    s.add_argument("--host", default="127.0.0.1")
    s.add_argument("--port", type=int, default=8000)
...
def main(argv: List[str] | None = None) -> None:
    ...
    logging.basicConfig(
        ...
        if args.verbose == 0
        else (logging.DEBUG if args.verbose >= 2 else logging.INFO),
